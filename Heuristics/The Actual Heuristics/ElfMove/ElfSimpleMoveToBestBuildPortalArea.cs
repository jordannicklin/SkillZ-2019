...
{
    class ElfSimpleMoveToBestBuildPortalArea : Heuristic
    {
        private List<Circle> heuristicObjectives = new List<Circle>();
        private float maxDistanceToCircle;

        public ElfSimpleMoveToBestBuildPortalArea(float weight, List<Circle> heuristicObjectives, float maxDistanceToCircle) : base(weight)
        {
        ...
        private List<Circle> GetNewListOfHeuristicObjectives()
        {
            ...
            foreach (Circle heuristicObjective in this.heuristicObjectives)
            {
                ...
                int countManaFountainsInArea = Constants.GameCaching.GetMyManaFountainsInArea(new Circle(heuristicObjective.GetCenter(), heuristicObjective.GetRadius() + Constants.Game.PortalSize + Constants.Game.ManaFountainSize )).Count;
